...
valuation_stockprice <- function(con, symbols, window_months = 18) {
  ...
  df_statement <- dplyr::tbl(con, "financial_stmts_build") |>
    ...
    dplyr::select(date = filingdate, symbol,
  ...
  # --- Jointure & propagation des valeurs financières aux dates de marché ---
  df <- df_price |>
  ...
  symbols_vec <- df |> dplyr::filter(!is.na(close)) |> dplyr::pull(symbol) |> unique() |> sort()
  ...
  df2 <- purrr::imap(
    split(df |> dplyr::filter(!is.na(close)) |> dplyr::arrange(symbol, date),
          df |> dplyr::filter(!is.na(close)) |> dplyr::arrange(symbol, date) |> dplyr::pull(symbol)),
    \(df_sym, sym) {
  ...
  # --- Bandes de prix : buy / caution / sell ---
  final <- df2 |>
    dplyr::mutate(
