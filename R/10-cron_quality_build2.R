# 10-cron_quality_build2.R — Reconstruire quality_build (version révisée)
#
# Version v2 de build_quality_build() : même logique (jointure de 5 sources,
# ratios de qualité, valorisation, buy ratios), mais écriture transactionnelle
# (TRUNCATE + append dans dbWithTransaction) au lieu d'un TRUNCATE non
# transactionnel.
...
#' passe, en joignant cinq sources : `financial_stmts_build` (dernière période
#' + moyennes ROA/ROE 5 ans), `valuation_build` (close, buy ratios),
#' `cagr_stmts_build` (PEG), `cies_profile_build` (secteur, industrie).
#'
...
#' @importFrom DBI dbGetQuery dbExecute dbWriteTable dbWithTransaction dbExistsTable
#'
#' @export
build_quality_build2 <- function(con) {
  ...
  ")
    ...
    # [5/5] Écriture transactionnelle (amélioration vs TRUNCATE non transactionnel)
    table_id <- DBI::Id(schema = "stocktools", table = "quality_build")
