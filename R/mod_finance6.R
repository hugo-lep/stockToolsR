# mod_finance6.R – Module Shiny : analyse boursière (version 6)
#
# Layout : trois onglets + sélecteur de compagnie partagé dans la navbar
#   Filtres       — filtres actifs + tableau des compagnies
#   check quality — QUALITÉ : Marges | ROA/ROE + Croissance | Structure financière
#   check price   — PRIX : Graphique buy/sell (2/3) | Valorisation + Dividendes (1/3)
#
...
#
# Fonctions exportées :
#   mod_finance6_ui1(id)         — nav_panel "Filtres"
#   mod_finance6_ui2(id)         — nav_panel "check quality"
#   mod_finance6_ui3(id)         — nav_panel "check price"
#   mod_finance6_ui(id)          — navset_bar wrapper complet
#   mod_finance6_server(id, con) — logique serveur

# UI 1 — Onglet Filtres

#' Onglet Filtres du module analyse boursière v6
#'
            ...
            bslib::layout_sidebar(
                ...
                bslib::card(
                    ...
                    bslib::card_header(
                        ...
                        shiny::textOutput(ns("n_filtrees6"), inline = TRUE)
                    ),
                    bslib::card_body(
                        style = "overflow-y:auto; padding:0.5rem;",
                        shiny::uiOutput(ns("table_filtrees6"))
                    )
                )
            )
        )
...
# UI 2 — Onglet check quality

#' Onglet check quality du module analyse boursière v6
#'
#' Section QUALITÉ uniquement : Marges | ROA/ROE + Croissance | Structure financière.
#'
#' @param id Identifiant du module
#' @return `bslib::nav_panel`
                ...
                bslib::card(
                    ...
                    bslib::card_header(
                        ...
                        shiny::tags$small(class = "text-muted",
                                          shiny::textOutput(ns("secteur_detail6"), inline = TRUE))
                    ),
                    bslib::card_body(
                        style = "overflow-y: auto; padding: 0.4rem; height: 100%;",
                        shiny::uiOutput(ns("tableau_marges6"))
                    )
                ),

                # CENTRE : ROA/ROE + Croissance
                bslib::card(
                    style = "height: 100%; margin: 0; min-height: 0;",
                    fill  = TRUE,
                    bslib::card_header(
                    ...
                    bslib::card_body(
                        style = "overflow-y: auto; padding: 0.4rem; height: 100%;",
                        shiny::uiOutput(ns("tableau_roa_roe6")),
                        shiny::tags$hr(class = "my-1"),
                        shiny::uiOutput(ns("tableau_croissance6"))
                    )
                ),

                # DROITE : Structure financière
                bslib::card(
                    style = "height: 100%; margin: 0; min-height: 0;",
                    fill  = TRUE,
                    bslib::card_header(
                    ...
                    bslib::card_body(
                        style = "overflow-y: auto; padding: 0.4rem; height: 100%;",
                        shiny::uiOutput(ns("tableau_structure6"))
                    )
                )
            )
        )
...
# UI 3 — Onglet check price

#' Onglet check price du module analyse boursière v6
#'
#' Section PRIX & INTÉRÊT : graphique buy/sell (2/3 gauche, pleine hauteur) +
#' Valorisation et Dividendes empilés (1/3 droite).
#'
#' @param id Identifiant du module
#' @return `bslib::nav_panel`
#' @importFrom shiny NS tags uiOutput plotOutput selectInput
#' @importFrom bslib nav_panel card card_header card_body
#' @importFrom bsicons bs_icon
#' @export
mod_finance6_ui3 <- function(id) {
    ns <- shiny::NS(id)
    ...
    bslib::nav_panel(
        ...
        shiny::tags$div(
            ...
            # Grille 2 colonnes : graphique (2/3) | valorisation + dividendes (1/3)
            shiny::tags$div(
                style = paste0(
                    "flex: 1; min-height: 0; ",
                ...
                # GAUCHE : Graphique + boîtes profit
                bslib::card(
                    style = "height: 100%; margin: 0; min-height: 0;",
                    fill  = TRUE,
                    bslib::card_header(
                    ...
                    bslib::card_body(
                        ...
                        # Sélecteur durée — superposé en haut à gauche
                        shiny::tags$div(
                            style = paste0(
                        ...
                        shiny::tags$div(
                            ...
                            shiny::plotOutput(ns("graph_buy6"), height = "100%")
                        ),
                        # Boîtes profit
                        shiny::uiOutput(ns("buy_signal_ui6"))
                    )
                ),

                # DROITE : Valorisation (haut) + Dividendes (bas)
                shiny::tags$div(
                    style = "display: flex; flex-direction: column; gap: 0.5rem; min-height: 0;",

                    bslib::card(
                        style = "flex: 1; margin: 0; min-height: 0;",
                        fill  = TRUE,
                        bslib::card_header(
                            class = "d-flex justify-content-between align-items-center py-1",
                            shiny::tags$b("Valorisation"),
                            shiny::tags$small(class = "text-muted",
                                              shiny::textOutput(ns("secteur_val6"), inline = TRUE))
                        ),
                        bslib::card_body(
                            style = "overflow-y: auto; padding: 0.4rem; height: 100%;",
                            shiny::uiOutput(ns("tableau_valorisation6"))
                        )
                    ),
                    ...
                    bslib::card(
                        ...
                        bslib::card_body(
                            style = "overflow-y: auto; padding: 0.4rem; height: 100%;",
                            shiny::uiOutput(ns("tableau_dividendes6"))
                        )
                    )
                )
            )
...
#' UI complète du module analyse boursière v6
#'
#' Inclut le sélecteur de compagnie partagé dans la navbar (visible dans tous les onglets).
#'
#' @param id Identifiant du module
#' @return `bslib::navset_bar`
#' @importFrom bslib navset_bar nav_spacer nav_item
#' @export
mod_finance6_ui <- function(id) {
    ns <- shiny::NS(id)
    bslib::navset_bar(
        id = ns("main_nav"),
        mod_finance6_ui1(id),
...
#' Serveur du module analyse boursière v6
#'
#' @param id  Identifiant du module
...
mod_finance6_server <- function(id, con) {
    shiny::moduleServer(id, function(input, output, session) {
        ...
        message("  [5/5] Jointure base_data6...")
        base_data6 <- quality |>
        ...
        cols_med6 <- c("roa", "roe", "roa_moy5", "roe_moy5",
                       "m_brut", "m_ebitda", "m_net", "fcf_rev",
                       "ratio_courant", "d_actif", "couv_interet",
                       "p_to_s_calc", "p_to_ebd_calc", "pe_calc", "peg_calc",
                       "pppi")

        secteur_med6 <- base_data6 |>
            dplyr::group_by(sector) |>
            dplyr::summarise(
                dplyr::across(dplyr::all_of(cols_med6),
                              ~ stats::median(.x, na.rm = TRUE)),
                .groups = "drop"
            )

        cagr_cols_med6 <- c("cagr_1_is_revenue", "cagr_1_is_ebitda",
                            "cagr_1_is_epsdiluted", "cagr_1_cf_freecashflow")

        secteur_cagr_med6 <- base_data6 |>
            dplyr::group_by(sector) |>
            dplyr::summarise(
                dplyr::across(dplyr::any_of(cagr_cols_med6),
                              ~ stats::median(.x, na.rm = TRUE)),
                .groups = "drop"
            )

        industrie_cagr_med6 <- base_data6 |>
        ...
        secteurs <- c("Tous", sort(unique(stats::na.omit(base_data6$sector))))
        ...
        filtres_actifs6 <- shiny::reactive({
            defs_static <- Filter(function(d) !isTRUE(d$div_sub), .filter_defs2)
            cats_static <- unique(sapply(defs_static, `[[`, "cat"))
        ...
        output$n_filtrees6 <- shiny::renderText({
            paste0(nrow(df_filtres6()), " compagnies")
        })

        output$table_filtrees6 <- shiny::renderUI({
            df <- df_filtres6() |> dplyr::arrange(ratio_moyen)
            if (nrow(df) == 0)
        ...
        # Clic sur une ligne → naviguer vers check quality
        shiny::observeEvent(input$clicked_symbol, {
            ...
            bslib::nav_select("main_nav", "check quality", session = session)
        })

        # Sync sélecteur symbole (liste filtrée)
        shiny::observe({
            df <- df_filtres6() |> dplyr::arrange(symbol)
            if (nrow(df) == 0) {
                shiny::updateSelectInput(session, "symbol",
        ...
        donnees_symbol6 <- shiny::reactive({
            shiny::req(input$symbol, nchar(input$symbol) > 0)
            base_data6 |> dplyr::filter(symbol == input$symbol)
        })

        val_hist6 <- shiny::reactive({
            shiny::req(input$symbol, nchar(input$symbol) > 0)
        ...
        val_hist6_filtered <- shiny::reactive({
            n_years <- as.numeric(shiny::req(input$duree_graph6))
            val_hist6() |>
                dplyr::filter(date >= Sys.Date() - lubridate::years(n_years))
        })

        val_latest6 <- shiny::reactive({
            df <- val_hist6()
            if (nrow(df) == 0) return(NULL)
            df |> dplyr::arrange(date) |> dplyr::slice_tail(n = 1)
        })

        # Barre d'info compagnie (navbar)

        output$info_bar6 <- shiny::renderUI({
            shiny::req(nrow(donnees_symbol6()) > 0)
            d       <- donnees_symbol6()
            cagr_1y <- d$cagr_1y
            close   <- d$close
            ...
            txt_chg <- if (is.na(cagr_1y)) "N/D"
                       else sprintf("%+.2f$ (%+.1f%%·1a)", chg_usd, cagr_1y * 100)
            ...
            shiny::tags$div(
                ...
                shiny::tags$span(class = "fs-6 fw-bold",
                                 scales::dollar(close, accuracy = 0.01)),
                shiny::tags$span(class = paste0("fw-semibold small text-", couleur), txt_chg)
            )
        })

        # Tableau Marges (check quality — gauche)

        output$secteur_detail6 <- shiny::renderText({
            shiny::req(nrow(donnees_symbol6()) > 0)
            sec <- donnees_symbol6()$sector
            if (is.na(sec)) return("")
            paste("vs.", sec)
        })

        output$tableau_marges6 <- shiny::renderUI({
            shiny::req(nrow(donnees_symbol6()) > 0)
            d   <- donnees_symbol6()
            sec <- d$sector
            ref <- secteur_med6 |> dplyr::filter(sector == sec)
            r   <- if (nrow(ref) == 1) ref else
                as.data.frame(as.list(stats::setNames(
                    rep(NA_real_, length(cols_med6)), cols_med6)))
        ...
        # Tableau ROA/ROE historique (check quality — centre, haut)

        output$tableau_roa_roe6 <- shiny::renderUI({
            shiny::req(nrow(donnees_symbol6()) > 0)
            d   <- donnees_symbol6()
            sec <- d$sector
            ref <- secteur_med6 |> dplyr::filter(sector == sec)
        ...
        # Tableau Croissance (check quality — centre, bas)

        output$tableau_croissance6 <- shiny::renderUI({
            shiny::req(nrow(donnees_symbol6()) > 0)
            d   <- donnees_symbol6()
            sec <- d$sector
            ind <- d$industry

            s_med <- secteur_cagr_med6   |> dplyr::filter(sector   == sec)
            i_med <- industrie_cagr_med6 |> dplyr::filter(industry == ind)
        ...
        # Tableau Structure financière (check quality — droite)

        output$tableau_structure6 <- shiny::renderUI({
            shiny::req(nrow(donnees_symbol6()) > 0)
            d   <- donnees_symbol6()
            sec <- d$sector
            ref <- secteur_med6 |> dplyr::filter(sector == sec)
        ...
        # Tableau Valorisation (check price — droite haut)

        output$secteur_val6 <- shiny::renderText({
            shiny::req(nrow(donnees_symbol6()) > 0)
            sec <- donnees_symbol6()$sector
            if (is.na(sec)) return("")
            paste("vs.", sec)
        ...
        output$tableau_valorisation6 <- shiny::renderUI({
            shiny::req(nrow(donnees_symbol6()) > 0)
            d   <- donnees_symbol6()
            sec <- d$sector
            ref <- secteur_med6 |> dplyr::filter(sector == sec)
        ...
        # Graphique bandes buy/sell (check price — gauche)

        output$graph_buy6 <- shiny::renderPlot({
            df  <- val_hist6_filtered()
            met <- if (is.null(input$metrique_graph6) ||
                       !input$metrique_graph6 %in% names(.metric_val_cols3))
                       "P/S" else input$metrique_graph6
        ...
        # Boîtes profit (check price — gauche, sous le graphique)

        output$buy_signal_ui6 <- shiny::renderUI({
            shiny::req(nrow(donnees_symbol6()) > 0)
            d  <- donnees_symbol6()
            vl <- val_latest6()

            met_actif <- if (is.null(input$metrique_graph6) ||
                             !input$metrique_graph6 %in% names(.metric_val_cols3))
                             "P/S" else input$metrique_graph6
            ...
            boite_profit <- function(b) {
                ...
                onclick_js  <- sprintf(
                    "Shiny.setInputValue('%s', '%s', {priority: 'event'})",
                    session$ns("metrique_graph6"), b$met
                )
                shiny::tags$div(
        ...
        # Tableau Dividendes (check price — droite bas)

        output$tableau_dividendes6 <- shiny::renderUI({
            shiny::req(nrow(donnees_symbol6()) > 0)
            d <- donnees_symbol6()
