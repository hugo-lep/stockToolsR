# 7-cron_tidy_stmts2_fcts.R — Reconstruction TTM de financial_stmts_build
#
# Version révisée de tidy_stmts() : même logique TTM (somme glissante 4
# trimestres, Q4 → FY, joints IS/BS/CF, marges), mais écriture transactionnelle
# (TRUNCATE + append dans dbWithTransaction) au lieu d'un overwrite non
# transactionnel. Si l'écriture échoue, la table n'est pas vidée.
...
#' - Le résultat est écrit dans `financial_stmts_build` en transaction
#'   (TRUNCATE + append) : si l'écriture échoue, la table existante est préservée.
#'
...
tidy_stmts2 <- function(con,
    ...
    bs_fy <- dplyr::tbl(con, dbplyr::in_schema("stocktools", "fy_balance_stmts_orig")) |>
        dplyr::collect()
    ...
    # Écriture transactionnelle (amélioration vs overwrite non transactionnel)
    table_id <- DBI::Id(schema = "stocktools", table = "financial_stmts_build")

    if (DBI::dbExistsTable(con, table_id)) {
        # TRUNCATE + insert dans une transaction : si l'écriture échoue,
        # le TRUNCATE est annulé → la table existante est préservée.
        DBI::dbWithTransaction(con, {
            DBI::dbExecute(con, "TRUNCATE TABLE stocktools.financial_stmts_build")
            DBI::dbWriteTable(con, table_id, final, append = TRUE)
        })
    } else {
        # Première création de la table
